...
void computeDescriptors(const torch::Tensor &mProb, const torch::Tensor &mDesc,
  ...
  Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> _desc(
      desc.data<float>(), desc.size(0), desc.size(1));
#ifdef USE_PCA
  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> _desc_new =
      (_desc.rowwise() - pca_mean) * pca_comp_T;
  local_descriptors =
      std::vector<float>(_desc_new.data(), _desc_new.data() + _desc_new.cols() * _desc_new.rows());
#else
