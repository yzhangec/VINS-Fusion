...
void PoseGraph::registerPub(ros::NodeHandle& n) {
  ...
  for (int i = 1; i < 10; i++) pub_path[i] = n.advertise<nav_msgs::Path>("path_" + to_string(i), 1);
        ...
        for (int j = 1; j < 5; j++) {
          if (i - j >= 0 && sequence_array[i] == sequence_array[i - j]) {
            ...
            ceres::CostFunction* vo_function = RelativeRTError::Create(
                relative_t.x(), relative_t.y(), relative_t.z(), relative_q.w(), relative_q.x(),
                relative_q.y(), relative_q.z(), 0.1, 0.01);
            problem.AddResidualBlock(vo_function, NULL, q_array[i - j], t_array[i - j], q_array[i],
                                     t_array[i]);
          }
        }
...
void PoseGraph::savePoseGraph() {
  ...
  for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
    ...
    fprintf(pFile,
            ...
            (*it)->index, (*it)->time_stamp.toSec(), VIO_tmp_T.x(), VIO_tmp_T.y(), VIO_tmp_T.z(),
...
void PoseGraph::loadPoseGraph() {
  ...
  while (fscanf(pFile,
    ...
    */
    ...
    KeyFrame* keyframe =
        new KeyFrame(ros::Time(time_stamp), index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index,
