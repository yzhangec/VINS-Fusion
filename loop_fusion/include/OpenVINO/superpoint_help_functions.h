...
void NMS2(std::vector<cv::Point2f> det, cv::Mat conf,
          std::vector<cv::Point2f> &pts, int border, int dist_thresh,
...
void getKeyPoints(const cv::Mat &prob, float threshold,
                  std::vector<cv::Point2f> &keypoints, int width, int height,
                  int max_num) {
  TicToc getkps;
  ...
  try {
    ...
    std::cerr << "Error in NMS2: " << e.what() << std::endl;
    return;
  }

  // printf(" NMS %f keypoints_no_nms %ld keypoints %ld/%d\n", ticnms.toc(),
...
void computeDescriptors(const torch::Tensor &mProb, const torch::Tensor &mDesc,
  ...
  local_descriptors = std::vector<float>(
      _desc_new.data(), _desc_new.data() + _desc_new.cols() * _desc_new.rows());
#else
  local_descriptors = std::vector<float>(
