...
void publishLastLandmarks(const ros::Time &ts, const std::vector<cv::Point3f> &landmarks) {
  std::cout << "publishLastLandmarks landmarks size: " << landmarks.size() << std::endl;
  pcl::PointCloud<pcl::PointXYZRGB> pointcloud;
...
void computePointsCovariance(const vector<cv::Point3f> &points_A,
  ...
  if (points_A.size() != points_B.size()) {
    cerr << "Error: Number of points in A and B must be equal!" << endl;
    exit(EXIT_FAILURE);
  }
...
void sync_process() {
  while (1) {
    ...
    nav_msgs::Odometry pose, last_pose;
    ...
    if (!img0_buf.empty() && !img1_buf.empty()) {
      ...
      if (time0 < time1 - 0.08) {
        ...
        if (pose.header.stamp.toSec() != 0) {
          ...
          if (!last_image0.empty() && !last_features.size() > 3) {
            ...
            Matrix<double, 6, 6> poseCovariance =
                computePoseCovariance(last_landmarks_cur, last_landmarks, R_w_b, t_w_b, cov);

            // Display result
            // cout << "Pose Covariance Matrix:\n" << poseCovariance << endl;
            // cout << "det(poseCovariance): " << poseCovariance.determinant() << endl;
            // cout << "trace(poseCovariance): " << poseCovariance.trace() << endl;

            // gtsam::Pose3 odometry = gtsam::Pose3(
            ...
            // double d_opt = marginals.marginalCovariance(graph.size()).determinant();
            // double t_opt = marginals.marginalCovariance(graph.size()).trace();
            // cout << "graph size: " << graph.size() << ", D-opt: " << d_opt << ", T-opt: " <<
            // t_opt
            //      << endl;

            std_msgs::Float32 cov_msg;
            cov_msg.data = cov.determinant();
            ...
            if (graph.size() == 0) {
              ...
              auto priorNoise = gtsam::noiseModel::Diagonal::Sigmas(gtsam::Vector6::Constant(0.01));
...
int main(int argc, char **argv) {
  ...
  pub_cov = n.advertise<std_msgs::Float32>("/vins_estimator/cov", 1);
