...
ros::Publisher pub_camera_pose, pub_camera_pose_inter, pub_latest_camera_pose;
...
void registerPub(ros::NodeHandle &n) {
  ...
  pub_camera_pose_inter = n.advertise<geometry_msgs::PoseStamped>("camera_pose_inter", 1000);
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q,
                       ...
                       const Eigen::Vector3d &omega, const Eigen::Matrix3d &ric,
                       const Eigen::Vector3d &tic, double t) {
  nav_msgs::Odometry odometry;
  odometry.header.stamp = ros::Time(t);
  odometry.header.frame_id = "world";

  Eigen::Vector3d w_T_center, v_center, a_center, omega_center;
  Eigen::Matrix3d w_R_center, R;
  ...
  Vector3d P_cam = P + R * tic;
  Quaterniond R_cam = Quaterniond(R * ric);
  ...
  pub_latest_camera_pose.publish(pose_cam);
}
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  static bool is_last_odom_avail = false;
  static Vector3d P_last;
  static Quaterniond R_last;
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    ...
    // if (is_last_odom_avail) {
    //   Vector3d P_inter = 0.5 * P_last + 0.5 * P;
    //   Quaterniond R_inter = R_last.slerp(0.5, R);
    //   odometry.header = header;
    //   odometry.header.frame_id = "world";
    //   odometry.header.stamp = t_last + (header.stamp - t_last) * 0.5;
    //   odometry.pose.position.x = P_inter.x();
    //   odometry.pose.position.y = P_inter.y();
    //   odometry.pose.position.z = P_inter.z();
    //   odometry.pose.orientation.x = R_inter.x();
    //   odometry.pose.orientation.y = R_inter.y();
    //   odometry.pose.orientation.z = R_inter.z();
    //   odometry.pose.orientation.w = R_inter.w();
    //   pub_camera_pose_inter.publish(odometry);
    // }

    // P_last = P;
    // R_last = R;
    // t_last = header.stamp;
    // is_last_odom_avail = true;
